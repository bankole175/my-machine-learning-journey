...
model = IrisClassifier(input_size=4, hidden_size=32, output_size=3)
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 5 == 0 or epoch == 0:
        print(f"Epoch {epoch + 1:3d} | Train Loss: {avg_train_loss:.4f} | Train F1: {train_f1:.3f} | Val Loss: {total_val_loss / len(val_loader):.4f} | Val F1: {val_f1:.3f} | {status}")
...
report = classification_report(all_labels, all_preds, target_names=['Setosa', 'Versicolour', 'Virginica'])
print(report)
...
checkpoint = torch.load("iris_checkpoint.pth")
